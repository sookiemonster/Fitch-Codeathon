...
const {
  changeItemOwner,
} = require("../../functions/items/patch/changeItemOwner.js");

const {
  changeItemStatus,
} = require("../../functions/items/patch/changeItemStatus.js");
...
router.patch("/:id/setStatus/:status", async (req, res) => {
  ...
  if (!status || (status !== "Fresh" && status !== "Dirty")) {
    return res.status(400).send({
      message:
        "Invalid or missing status (status can only be 'Fresh' or 'Dirty')",
    });
  }
...
router.patch("/:id/setOwner/:ownerId", async (req, res) => {
  ...
  try {
    const message = await changeItemOwner(
      parseInt(id),
      parseInt(ownerId),
      force
    );
    ...
    if (error.message === "Entity not found") {
      return res
        .status(404)
        .send({ message: "Entity with given ID not found" });
    }
...
router.post("/create", async (req, res) => {
  ...
  if (!name && !status) {
    return res
      .status(400)
      .send({ message: "Invalid or missing name and status" });
  }

  if (
    capitalizeFirstLetter(name) !== "Plate" &&
    capitalizeFirstLetter(name) !== "Cup"
  ) {
