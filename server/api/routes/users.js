...
const { getUserHistory } = require('../../functions/users/get/getUserHistory.js');
...
const { addUserPoints } = require('../../functions/users/patch/addUserPoints.js');
...
router.patch('/:id/items/:item/add', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Item was successfully added to user Current Items list!', userId : id, itemId : item });
    } catch (error) {
...
router.patch('/:id/history/:item/add', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Item was successfully added to user History list!', userId : id, itemId : item });
    } catch (error) {
        if (error.message === "Item not found") {
...
router.patch('/:id/discounts/:discount/add', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Discount was successfully added to user discounts list!', userId : id, discountId : discount });
    } catch (error) {
...
router.patch('/:id/discounts/:discount/remove', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Discount was successfully removed from user discounts list!', userId : id, discountId : discount });
    } catch (error) {
...
router.patch('/:id/points/add',async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Points were successfully added to user points list!', userId : id});
    } catch (error) {
        if (error.message === "User not found") {
            return res.status(404).send({ message : 'User not found' });
        }
...
router.patch('/:id/points/remove', async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ message : 'Points were successfully removed from user points list!', userId : id});
    } catch (error) {
        if (error.message === "User not found") {
            return res.status(404).send({ message : 'User not found' });
        }
